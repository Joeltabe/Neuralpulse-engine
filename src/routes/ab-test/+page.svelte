    ...
    try {
      if (mode === 'text') {
        const res = await fetch('/api/analyze/text', {
          ...
          body: JSON.stringify({ texts: textVariants.filter(t => t.trim()), variant_names: textVariants.map((_, i) => `Variant ${i+1}`) })
        });
        ...
        const res = await fetch(`/api/analyze/${mode}`, { method: 'POST', body: form });
        const data = await res.json();
        if (data.success) result = data.data;
        else showError(data.error || 'Failed');
      }
    ...
    <Card>
      ...
      {#if files.length > 0}
        <div class="mt-3 space-y-1">
          {#each files as f}<div class="text-sm text-white/60">{f.name}</div>{/each}
        </div>
      {/if}
    </Card>
  {/if}
